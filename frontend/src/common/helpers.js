...
export const normalizeTask = task => {
    return {
        ...task, status: task.statusId ? taskStatuses[task.statusId] : '', timeStatus: getTimeStatus(task.dueDate)
    };
};
...
export const getImage = (image) => {
    ...
    console.log('URL:', new URL(`src/assets/img/${image}`, window.location.origin).href)
    return new URL(`src/assets/img/${image}`, window.location.origin).href
}
